...
//      mmtb.show(200)
      ...
      val mytb = spark.sql("select min(id) maxindex,uid,cid,rid,rt,tag,min(ct) minct,max(ct) maxct,maxsp from mmtb group by id,uid,cid,rid,rt,tag,maxsp order by uid,cid,rid,rt")
//      mytb.show(200)
      //创建临时表
      mytb.createOrReplaceTempView("mytb")
      val void_dbtb=spark.sql("select maxindex,uid,cid,rid,rt,minct,maxct,maxsp from mytb order by uid,cid,rid")
      //    存入数据库
      insertMySQL("session_course_resource", void_dbtb)
    ...
    getDBData("r_liveop")
//    val r_offliveop_jdbcDF = getDate("r_offliveop", spark)
//    r_offliveop_jdbcDF.createOrReplaceTempView("r_offliveop")
//    getDBData("r_offliveop")  //缺少cid
    val r_pictxtop_jdbcDF = getDate("r_pictxtop", spark)
    r_pictxtop_jdbcDF.createOrReplaceTempView("r_pictxtop")
    getDBData("r_pictxtop")
//    val r_ackop_jdbcDF = getDate("r_ackop", spark)
//    r_ackop_jdbcDF.createOrReplaceTempView("r_ackop")
//    getDBData("r_ackop")//缺少cid
